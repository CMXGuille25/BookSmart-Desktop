...
import PantallaTransicion from '../PantallaTransicion/PantallaTransicion';
...
const Sidebar = () => {
  ...
  const handleLogout = async () => {
    try {
      ...
      if (token) {
        await fetch('http://localhost:3333/api/auth/token/revoke', {
          method: 'POST',
          headers: {
      ...
      // Puedes mostrar un mensaje si quieres, pero el logout debe continuar
    } finally {
      // Limpia todo lo relacionado al usuario y token
      localStorage.removeItem('token');
      localStorage.removeItem('user');
      localStorage.removeItem('temp_token');
      localStorage.removeItem('pending_user');
      setShowTransicion(true);
      setTimeout(() => {
  ...
  return (
    <>
      <aside className="sidebar">
        ...
        <div className="sidebar-divider" />
        <div className="menu-item cerrar-sesion" onClick={handleLogout} style={{ cursor: 'pointer' }}>
      ...
      {showTransicion && (
        <PantallaTransicion icono={logo1} soloIcono={true} />
      )}
    </>
  );
};
